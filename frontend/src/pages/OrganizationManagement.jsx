...
// Função para buscar o perfil do usuário (que contém o organization_id)
...
// Função corrigida para buscar a organização baseada no perfil do usuário
const fetchUserOrganization = async () => {
  try {
    ...
    if (!profile || !profile.organization) {
      console.log('No organization found in profile');
      return null;
    }

    // Agora buscar a organização específica do usuário
    const response = await api.get(`/organizations/${profile.organization}/`);
    console.log('Organization loaded:', response.data);
    return response.data;
  } catch (error) {
    console.error('Error fetching user organization:', error);
    
    // Se falhar, tentar buscar todas as organizações e pegar a primeira (fallback)
    try {
...
const OrganizationManagement = () => {
  ...
  // Query corrigida para buscar a organização do usuário
  const {
    data: organization, 
  ...
  const createNewMemberAndSetPermissionsMutation = useMutation({
    mutationFn: async ({ invitationData, fullPermissionsData }) => {
      const addMemberPayload = {
        ...
        is_admin: fullPermissionsData.is_admin,
  ...
  const isLoadingOverall = isLoadingOrganization || isLoadingMembers || isLoadingUsers ||
    ...
    createNewMemberAndSetPermissionsMutation.isPending || removeMemberMutation.isPending || 
    updateMemberMutation.isPending;

  if (isLoadingOrganization || isLoadingUsers) {
    return (
        ...
        <motion.div variants={itemVariants} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            ...
            <p style={{ fontSize: '1rem', color: 'rgba(191, 219, 254, 1)', margin: 0 }}>
              Configure sua organização e gerencie membros.
            </p>
            {organization && (
              <p style={{ fontSize: '0.875rem', color: 'rgba(147, 51, 234, 1)', margin: '0.5rem 0 0 0', fontWeight: '500' }}>
                Organização atual: {organization.name}
              </p>
            )}
          </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
                ...
                <div>
                  ...
                  <p style={{ margin: 0, fontSize: '0.875rem', color: 'rgb(191,219,254)' }}>
                    {editingOrganization ? "Atualize os detalhes da sua organização." : "Crie uma nova organização."}
                  </p>
                </div>
              </div>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
                <div style={{ padding: '0.5rem', backgroundColor: 'rgba(52,211,153,0.2)', borderRadius: '12px' }}>
                  <UserPlus style={{ color: 'rgb(52,211,153)' }} size={20} />
                </div>
                <div>
                  <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: '600' }}>
                    Adicionar Novo Membro (Passo 1/2)
                  </h3>
                  <p style={{ margin: 0, fontSize: '0.875rem', color: 'rgb(191,219,254)' }}>
                    Insira o código de convite do utilizador.
                  </p>
                </div>
              </div>
              <InvitationCodeForm
          ...
          {memberCreationStep === 'permissions' && organization && !showOrganizationForm && (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
                <div style={{ 
                  padding: '0.5rem', 
                  backgroundColor: editingMember ? 'rgba(147,51,234,0.2)' : 'rgba(52,211,153,0.2)', 
                  borderRadius: '12px' 
                }}>
                  {editingMember ? 
                    <Edit style={{ color: 'rgb(147,51,234)' }} size={20} /> : 
                    <Shield style={{ color: 'rgb(52,211,153)' }} size={20} />
                  }
                </div>
                <div>
                  <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: '600' }}>
                    {editingMember ? 
                      `Editar Permissões de ${editingMember.username}` : 
                      'Definir Permissões (Passo 2/2)'
                    }
                  </h3>
                  <p style={{ margin: 0, fontSize: '0.875rem', color: 'rgb(191,219,254)' }}>
                    Configure o acesso e as permissões do membro.
                  </p>
                </div>
              </div>
              <MemberPermissionsForm
                member={editingMember || (newMemberDataFromInvitation ? { 
                  ...newMemberDataFromInvitation, 
            ...
            <motion.div variants={itemVariants} style={{ ...glassStyle, padding: '1.5rem', marginBottom: '2rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
                <div style={{ padding: '0.5rem', backgroundColor: 'rgba(147,51,234,0.2)', borderRadius: '12px' }}>
                  <Building style={{ color: 'rgb(147,51,234)' }} size={20} />
                </div>
              ...
              <div style={{ 
                ...
                <p><strong>ID:</strong> <span style={{ fontFamily: 'monospace', fontSize: '0.75rem' }}>{organization.id}</span></p>
              </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} style={{ ...glassStyle, padding: 0, overflow: 'hidden' }}>
              <div style={{ 
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  <div style={{ padding: '0.5rem', backgroundColor: 'rgba(52,211,153,0.2)', borderRadius: '12px' }}>
                    <Users style={{ color: 'rgb(52,211,153)' }} size={20} />
                  </div>
                  <div>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem' }}>
            ...
            <div>
              ...
              <p style={{ 
                ...
                Uma organização permite agrupar utilizadores, clientes e tarefas. Administradores podem gerir membros, 
                definir permissões detalhadas e configurar as definições gerais da organização. Utilize códigos de 
                convite para adicionar novos membros de forma segura.
              </p>
            </div>
          </div>
        </motion.div>
